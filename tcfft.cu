...
__global__ void single_256(float *data, float *dft, float *twiddle) {
    ...
    wmma::load_matrix_sync(frag_dft_real_1, dft, 32);
    wmma::load_matrix_sync(frag_dft_real_2, dft+16, 32);
    wmma::load_matrix_sync(frag_dft_imag_1, dft+256, 16);
    wmma::load_matrix_sync(frag_dft_imag_2, dft+384, 16);
    // 加载twiddle矩阵
    ...
    wmma::load_matrix_sync(frag_twiddle_imag, twiddle, 16, wmma::mem_row_major);
    // 定义输入输出矩阵
    ...
    // 读出指定位置的矩阵
    int warp_start = 0;
    wmma::load_matrix_sync(frag_data_real_1, data, 32);
    wmma::load_matrix_sync(frag_data_real_2, data + 16, 32);
    wmma::load_matrix_sync(frag_data_imag_1, data + 256, 16);
    wmma::load_matrix_sync(frag_data_imag_2, data + 384, 16);
    ...
    for (int i = 0; i < frag_out_real.num_elements; i++){
        ...
        c = frag_out_real.x[i] * frag_twiddle_imag.x[i];
        d = frag_out_imag.x[i] * frag_twiddle_real.x[i];
        frag_out_real.x[i] = a - b;
        frag_out_imag.x[i] = c + d;
    }
    __syncthreads();
    // 将计算结果转置并重新存储回frag_data
    ...
    wmma::load_matrix_sync(frag_data_real_1, smem, 32);
    wmma::load_matrix_sync(frag_data_real_2, smem + 16, 32);
...
extern "C" void launch_half_256(half* data, tcfftHandle plan) {
    // 调用CUDA核心 
    half_256<<<1, 32, sizeof(half)*plan.nx*2*plan.batch>>>(data, (half *)plan.dft_real, (half *)plan.dft_imag, (half *)plan.twiddle_real, (half *)plan.twiddle_imag);
}
